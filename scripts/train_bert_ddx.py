"""
scripts/train_bert_ddx.py

Fine-tunes DistilBERT on DDXPlus with KL divergence loss for differential diagnosis.

Novel contribution: instead of cross-entropy on a single label, we train the model
to predict a probability distribution over 49 diseases matching the gold
DIFFERENTIAL_DIAGNOSIS distributions from the DDXPlus dataset.

Saves to: models/ddxplus_model/
...
WEIGHT_DECAY = 0.01
WARMUP_RATIO = 0.1
ALPHA        = 0.9    # weight for KL loss; (1-ALPHA) for auxiliary CE loss
GRAD_ACCUM   = 4      # effective batch size = 64

ROOT_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DDX_DIR    = os.path.join(ROOT_DIR, 'data', 'ddxplus')
MODEL_DIR  = os.path.join(ROOT_DIR, 'models', 'ddxplus_ablation_09')
...
# ── Load label map ────────────────────────────────────────────────────────────
# Label map is always read from the base ddxplus_model dir (same 49 classes for all runs)
_BASE_MODEL_DIR = os.path.join(ROOT_DIR, 'models', 'ddxplus_model')
lm_path = os.path.join(_BASE_MODEL_DIR, 'ddxplus_label_map.json')
...
metrics = {
    'model': 'DistilBERT + KL Divergence',
